...
def plot(
    ...
    if self.is_stator:
        ...
        custom_color = colors.get("stator")
    else:
        Lam_Name = "Rotor"
        lam_color = ROTOR_COLOR
        custom_color = colors.get("rotor")

    if custom_color:
        lam_color = custom_color

    if colors.get("magnet"):
        mag_color = colors.get("magnet")
    else:
        mag_color = MAGNET_COLOR
    ...
    for surf in surf_list:
        ...
        if LAM_LAB in label_dict["surf_type"]:
            ...
            if not is_lam_only:
                patches.extend(
                    surf.get_patches(color=mag_color, is_edge_only=is_edge_only)
                )
        else:
    ...
    if not is_edge_only:
        ...
        if not is_lam_only:
            patch_leg.append(Patch(color=mag_color))
